...
egamma_features = df_egamma[feature_list].values
zprime_features = df_zprime[feature_list].values
muons_features  = df_muons[feature_list].values
...
plot_histogram(4, r"Invariant mass of lepton and b-jet, Z' data set", r'$m_{b\ell}$ [GeV]',
    r'No. of events without top', zprime_features[pred_0_zprime[0],9][:], bins_, "zprime_hist_0_inv.pdf",
...
plot_histogram(5, r"Invariant mass of lepton and b-jet, Z' data set", r'$m_{b\ell}$ [GeV]',
    r'No. of events with top', zprime_features[pred_1_zprime[0],9][:], bins_, "zprime_hist_1_inv.pdf",
    xlim_inv, vertical=np.sqrt(170**2 - 80.385**2), verticallabel=r'$\sqrt{m_t^2 - m_W^2}$')
...
plt.figure(1)
plt.hist(muons_features[pred_1_muons[0],9], bins=bins_, range=xlim_trans, label=r'$e \mu$ With $t$')
plt.hist(egamma_features[pred_1_egamma[0],9], bins=bins_, range=xlim_trans, label=r'$e\gamma$ With $t$')
plt.hist(muons_features[pred_0_muons[0],9], bins=bins_, range=xlim_trans, label=r'$e\mu$ Without $t$')
plt.hist(egamma_features[pred_0_egamma[0],9], bins=bins_, range=xlim_trans, label=r'$e\gamma$ Without $t$')
...
plt.legend()
plt.savefig('egamma_emuons_invmass_01.pdf')